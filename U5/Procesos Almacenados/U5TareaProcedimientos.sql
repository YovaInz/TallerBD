...
CREATE PROC SP_ORDENES_ANIO @anio INT, @ctes NVARCHAR(MAX) = '' OUTPUT, @ords NVARCHAR(MAX) = '' OUTPUT AS
SET @ctes = ''
SET @ords = ''
...
--* 2022	ANA, CARLOS
CREATE PROC SP_CLIENTES_ANIO AS
...
--* 8.- SP QUE RECIBA UN AÑO COMO PARAMETRO DE ENTRADA, QUE REGRESE UNA TABLA CON DOS COLUMNAS: MES, FOLIOS QUE SE VENDIERON ESE MES. 
...
CREATE PROC SP_ORDENES_MES @ANIO INT AS
DECLARE @mes INT, @folios NVARCHAR(MAX)

CREATE TABLE #meses (mes NVARCHAR(10), folios NVARCHAR(MAX))
SET @mes = 1
...
BEGIN
    ...
    SELECT @folios = STRING_AGG(CONVERT(VARCHAR(5), OrderID), ', ') FROM Orders 
    WHERE MONTH(OrderDate) = @mes AND YEAR(OrderDate) = @ANIO

    INSERT #meses VALUES (DATENAME(MONTH, DATEFROMPARTS(@ANIO, @mes, 1)), ISNULL(@folios, ''))
...
CREATE PROC SP_EMPLEADO_TERRITORIOS @emp INT, @territorios NVARCHAR(200) OUTPUT AS
SELECT @territorios = ''
...
WHERE ET.EmployeeID = @emp
GO
-- Ejecucion
DECLARE @T NVARCHAR(200)
EXEC SP_EMPLEADO_TERRITORIOS 2, @T OUTPUT
SELECT @T
GO
--* 10.- DE LA SIGUIENTE TABLA ESTUDIANTES, REALIZAR UN PROCESO QUE ELIMINE LOS REGISTROS DUPLICADOS.
...
/*Insert into ESTUDIANTES ([FirstName],[LastName],[Country] )values
    ...
    ('Ana','Lopez','USA'),
    ('Ana','Lopez','USA')*/
CREATE PROC SP_ELIMINAR_DUPLICADOS AS
CREATE TABLE #estudiantes (FirstName varchar(100), LastName varchar(100), Country varchar(100)) 
INSERT #estudiantes 
