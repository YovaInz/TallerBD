...
SELECT h.ZONA, 
CITAS = COUNT(CI.FOLIO),
HOSPITALES = COUNT(distinct h.HOSPID), 
CONSULTORIOS = COUNT(distinct CO.CONID)
FROM vw_hospitales H 
LEFT OUTER JOIN vw_consultorios CO ON h.ZONA = CO.ZONA AND H.HOSPID = CO.HOSPID
LEFT OUTER JOIN vw_citas CI ON CI.CONID = CO.CONID
GROUP BY h.ZONA